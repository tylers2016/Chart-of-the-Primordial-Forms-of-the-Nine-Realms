...
def link_data(md_data: ProcessedData, shp_data: Dict[str, gpd.GeoDataFrame]) -> ProcessedData:
    ...
    """
    print("Starting data linking...")
    if not md_data or not shp_data.get("province") or not shp_data.get("city") or not shp_data.get("district"):
        print("Missing data for linking. Aborting.")
        return md_data

    # Create lookup tables from shapefiles for easier matching by name
    # We will primarily match by name and then assign adcode and geometry
    # (Adcode is king, but MD parsing gives names first)
    ...
    for province_md in md_data:
        prov_name_md = normalize_name(province_md["name"])
        # Match province
        matched_prov_shp = gdf_provinces[gdf_provinces['pr_name'].apply(normalize_name) == prov_name_md]
        if not matched_prov_shp.empty:
            province_md["adcode"] = matched_prov_shp.iloc[0]["pr_adcode"]
            province_md["geometry"] = matched_prov_shp.iloc[0].geometry
            print(f"  Linked Province: {prov_name_md} -> ADCODE {province_md['adcode']}")

            for city_md in province_md["children"]:
                city_name_md = normalize_name(city_md["name"])
                # Match city within the current province's adcode prefix
                prov_adcode_prefix = str(province_md["adcode"])[:2] # e.g., "13" for Hebei
                
                # Filter cities by matching province adcode first, then by name
                # Assuming city adcodes start with province adcode's first 2 digits.
                # ct_adcode often is what we want. pr_adcode in city.shp is parent.
                # gdf_cities['pr_adcode'] links city to province.
                
                potential_cities_shp = gdf_cities[gdf_cities['pr_adcode'] == province_md["adcode"]]
                matched_city_shp = potential_cities_shp[potential_cities_shp['ct_name'].apply(normalize_name) == city_name_md]

                if not matched_city_shp.empty:
                    city_md["adcode"] = matched_city_shp.iloc[0]["ct_adcode"]
                    city_md["geometry"] = matched_city_shp.iloc[0].geometry
                    city_md["parent_adcode"] = province_md["adcode"] # Confirm parent adcode
                    print(f"    Linked City: {city_name_md} (under {prov_name_md}) -> ADCODE {city_md['adcode']}")

                    # Linking districts (H3 content is currently merged into city_md["text_detail"])
                    # If we had district objects in md_data:
                    # for district_md in city_md["children"]:
                    #   dist_name_md = normalize_name(district_md["name"])
                    #   potential_districts_shp = gdf_districts[gdf_districts['ct_adcode'] == city_md["adcode"]]
                    #   matched_district_shp = potential_districts_shp[potential_districts_shp['dt_name'].apply(normalize_name) == dist_name_md]
                    #   if not matched_district_shp.empty:
                    #       district_md["adcode"] = matched_district_shp.iloc[0]["dt_adcode"]
                    #       district_md["geometry"] = matched_district_shp.iloc[0].geometry
                    #       print(f"      Linked District: {dist_name_md} -> ADCODE {district_md['adcode']}")
                    #   else:
                    #       print(f"      WARNING: District not found in shapefile: {dist_name_md} under City ADCODE {city_md['adcode']}")
                else:
                    print(f"    WARNING: City not found in shapefile: {city_name_md} under Province {prov_name_md} (ADCODE {province_md['adcode']})")
        else:
            print(f"  WARNING: Province not found in shapefile: {prov_name_md}")
            
    print("Data linking finished.")
    return md_data
...
def main_data_processing():
    ...
    # 1. Parse Markdown
    # The markdown parser needs significant refinement for accurate text attribution
    # to "general" vs "detail" and for creating distinct district entries.
    # For now, it gives a basic structure.
    parsed_md = parse_markdown(MD_FILE_PATH)
    if not parsed_md:
        print("Markdown parsing failed or returned no data. Exiting.")
        return None

    # 2. Load Shapefiles
    shapefile_data = load_shapefiles()
    if not all(key in shapefile_data for key in ["country", "province", "city", "district"]):
        print("Shapefile loading incomplete. Exiting.")
        return None

    # 3. Link Markdown Data with Shapefile Data
    # This also needs refinement, especially if names are not exact matches
    # or if adcodes are not perfectly aligned.
    linked_data_structure = link_data(parsed_md, shapefile_data)
    
    # At this point, linked_data_structure should contain a list of provinces,
    # each with its cities, and each entity (prov, city) potentially having
    # adcode, geometry, and text. District linking is conceptual here.

    # For demonstration, print some results
    # if linked_data_structure:
    #     for province in linked_data_structure:
    #         print(f"Province: {province['name']}, ADCODE: {province.get('adcode')}, Has Geometry: {province.get('geometry') is not None}")
    #         for city in province.get('children', []):
    #             print(f"  City: {city['name']}, ADCODE: {city.get('adcode')}, Has Geometry: {city.get('geometry') is not None}")
                # Further nesting for districts if they were parsed as separate objects
