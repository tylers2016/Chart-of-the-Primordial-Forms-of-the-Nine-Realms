...
class MapViewer(QWidget):
    ...
    def display_geometry(self, geometry_to_plot: Optional[Any], fit_bounds=True): # Renamed for clarity
        """Clears current plot and displays new geometry data."""
        self.ax.clear()

        # Check if geometry_to_plot is a valid geometry object (from Shapely, typically)
        # or a GeoSeries/GeoDataFrame that can be plotted.
        # A simple None check is often sufficient if we trust the data source.
        # For individual Shapely geometries, they have an .is_empty property.
        # For GeoSeries/GeoDataFrame, they have an .empty attribute.

        can_plot = False
        if geometry_to_plot is not None:
            if hasattr(geometry_to_plot, 'empty'): # It's likely a GeoDataFrame or GeoSeries
                if not geometry_to_plot.empty:
                    can_plot = True
            elif hasattr(geometry_to_plot, 'is_empty'): # It's likely a Shapely geometry object
                if not geometry_to_plot.is_empty:
                    can_plot = True
            else: # Fallback for other plottable types, or assume plottable if not None
                can_plot = True # This might need to be more robust depending on what 'geometry' can be

        if can_plot:
            try:
                # If it's already a GeoDataFrame or GeoSeries, plot directly
                if isinstance(geometry_to_plot, (gpd.GeoDataFrame, gpd.GeoSeries)):
                    plotted_geom = geometry_to_plot
                else:
                    # If it's a Shapely geometry, wrap it in a GeoSeries to plot
                    # and to get .total_bounds. A CRS might be needed for accurate plotting.
                    plotted_geom = gpd.GeoSeries([geometry_to_plot])
                    # If you know the CRS, assign it:
                    # plotted_geom.crs = "EPSG:4326" # Example, use actual CRS if known
                
                plotted_geom.plot(ax=self.ax, edgecolor='black', color='lightblue')
                ...
                if fit_bounds:
                    ...
                    # Handle cases where geometry might be a point (minx=maxx)
